...
function processSum(numberList, callback) {
  const arraySum = numberList.reduce(function(a,b){
    return a+b;
  }, 0);
  return callback(arraySum);
}
...
function processContains(item, list, callback) {
  let listCheck = list.includes(item)
  return callback(listCheck);
}
...
function getFullNames(runners) {
  ...
  runners.forEach((runner) => {
    const {first_name, last_name} = runner; //object deconstruction
    fullNames.push(`${last_name}, ${first_name}`)
  });
  return fullNames;
}
...
function firstNamesAllCaps(runners) {
  const toCaps = runners.map(function(items){
